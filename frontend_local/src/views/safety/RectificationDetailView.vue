    ...
    <div v-if="order" class="layout">
      ...
      <div class="side-col">
        <div class="card">
          ...
          <!-- 验证 -->
          <div v-if="order.status === 'verifying' && canAssignOrVerify" class="action-block">
...
// 兼容旧逻辑：安全员 或 整改分派人 或 当前工单的指派验证人 都有分派/验证权限
const canAssignOrVerify = computed(() => {
  if (isOfficer.value) return true
  if (userStore.isAssigner) return true  // 检查整改分派人权限
