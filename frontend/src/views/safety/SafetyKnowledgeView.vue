<template>
  <div class="page-container">
    <div class="page-header">
      <div>
        <h2 class="page-title">安全知识库</h2>
        <p class="page-subtitle">查询烟草制丝安全规范与操作规程</p>
      </div>
    </div>
    ...
    <div class="search-card">
      <div class="search-row">
        <el-input
          v-model="question"
          placeholder="输入问题，如：动火作业安全规程是什么？"
          size="large"
          clearable
    ...
    <div v-else class="initial-state">
      <div class="initial-icon">
        <svg width="56" height="56" viewBox="0 0 56 56" fill="none" stroke="currentColor" stroke-width="1.2" opacity="0.2">
          <path d="M28 4L6 14v14c0 14 9.33 26 22 29 12.67-3 22-15 22-29V14L28 4z"/>
          <line x1="20" y1="28" x2="26" y2="34"/>
          <line x1="26" y1="34" x2="38" y2="22"/>
        </svg>
      </div>
      <p class="initial-text">输入问题，从安全知识库中检索相关内容</p>
      <p class="initial-sub">知识库包含安全操作规程、应急预案、危险源管理等内容</p>
    </div>
  </div>
</template>
...
async function handleSearch() {
  ...
  try {
    const { data } = await http.post('/api/safety/knowledge/search/', {
      question: q,
      top_n: topN.value,
    })
